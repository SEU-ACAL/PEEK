...
import boom.peek.ifu.{GetPCFromFtqIO}
import boom.peek.util.{ImmGen, IsKilledByBranch, BranchKillableQueue, BoomCoreStringPrefix}
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val io = IO(new Bundle {
    ...
    val lsu_io = if (hasMem) Flipped(new boom.peek.lsu.LSUExeIO) else null
