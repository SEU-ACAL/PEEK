      ...
      (0 until n).map { i =>
        BoomTileAttachParams(
          tileParams = BoomTileParams(
            ...
            dcache = Some(
              DCacheParams(rowBits = 128, nSets=64, nWays=8, nMSHRs=4, nTLBWays=8)
            ),
            icache = Some(
 ...
 * 4-wide PEEK BOOM.
 */
class WithNPEEKBooms(n: Int = 1, overrideIdOffset: Option[Int] = None) extends Config(
  new WithTAGELBPD (1024)++ // Default to TAGE-L BPD, but with less BIM size.
        ...
        BoomTileAttachParams(
          tileParams = BoomTileParams(
            core = BoomCoreParams(
              ...
              numRobEntries = 84,
              issueParams = Seq(
                IssueParams(issueWidth=2, numEntries=12, iqType=IQT_MEM.litValue, dispatchWidth=4),
                IssueParams(issueWidth=4, numEntries=24, iqType=IQT_INT.litValue, dispatchWidth=4),
                IssueParams(issueWidth=2, numEntries=16, iqType=IQT_FP.litValue , dispatchWidth=4)),
              numIntPhysRegisters = 100,
              numFpPhysRegisters = 96,
              numLdqEntries = 20,
              numStqEntries = 20,
              maxBrCount = 20,
              numFetchBufferEntries = 20,
              // enablePrefetching = true,
              ftq = FtqParameters(nEntries=24),
              fpu = Some(freechips.rocketchip.tile.FPUParams(sfmaLatency=4, dfmaLatency=4, divSqrt=true))
            ),
            dcache = Some(
              DCacheParams(rowBits = 128, nSets=64, nWays=4, nMSHRs=4, nTLBWays=8)
            ),
            icache = Some(
