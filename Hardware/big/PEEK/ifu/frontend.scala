...
import boom.peek.exu.{CommitExceptionSignals, BranchDecode, BrUpdateInfo, BranchDecodeSignals}
...
class BoomFrontend(val icacheParams: ICacheParams, staticIdForMetadataUseOnly: Int)(implicit p: Parameters) extends LazyModule
{
  ...
  val icache = LazyModule(new boom.peek.ifu.ICache(icacheParams, staticIdForMetadataUseOnly))
