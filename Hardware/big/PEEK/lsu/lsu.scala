...
import boom.peek.exu.{BrUpdateInfo, Exception, FuncUnitResp, CommitSignals, ExeUnitResp}
import boom.peek.util.{BoolToChar, AgePriorityEncoder, IsKilledByBranch, GetNewBrMask, WrapInc, IsOlder, UpdateBrMask}
...
class LSUExeIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val iresp    = new DecoupledIO(new boom.peek.exu.ExeUnitResp(xLen))
  val fresp    = new DecoupledIO(new boom.peek.exu.ExeUnitResp(xLen+1)) // TODO: Should this be fLen?
}
