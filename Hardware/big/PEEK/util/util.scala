...
import boom.peek.common.{MicroOp}
import boom.peek.exu.{BrUpdateInfo}
...
object UpdateBrMask
{
  ...
  def apply[T <: boom.peek.common.HasBoomUOP](brupdate: BrUpdateInfo, bundle: T): T = {
    val out = WireInit(bundle)
    out.uop.br_mask := GetNewBrMask(brupdate, bundle.uop.br_mask)
    out
  }
  def apply[T <: boom.peek.common.HasBoomUOP](brupdate: BrUpdateInfo, bundle: Valid[T]): Valid[T] = {
    val out = WireInit(bundle)
...
object ImmGen
{
  import boom.peek.common.{LONGEST_IMM_SZ, IS_B, IS_I, IS_J, IS_S, IS_U}
 ...
 */
class BranchKillableQueue[T <: boom.peek.common.HasBoomUOP](gen: T, entries: Int, flush_fn: boom.peek.common.MicroOp => Bool = u => true.B, flow: Boolean = true)
  (implicit p: org.chipsalliance.cde.config.Parameters)
  extends boom.peek.common.BoomModule()(p)
  with boom.peek.common.HasBoomCoreParameters
{
  val io = IO(new Bundle {
