...
import freechips.rocketchip.subsystem.{TilesLocated, NumTiles, HierarchicalLocation, RocketCrossingParams, SystemBusKey, CacheBlockBytes, RocketTileAttachParams,RocketTilePeekAttachParams, RocketTileNpuAttachParams, RocketTileAttachParamsBB, InSubsystem, InCluster, HierarchicalElementMasterPortParams, HierarchicalElementSlavePortParams, CBUS, CCBUS, ClustersLocated, TileAttachConfig, CloneTileAttachParams}
...
import freechips.rocketchip.peek.RocketTilePeekParams
...
class WithNGCCheckers(n: Int, overrideIdOffset: Option[Int] = None,crossing: RocketCrossingParams = RocketCrossingParams()) extends Config((site, here, up) => {
  ...
  case TilesLocated(InSubsystem) => {
    ...
    val checker = RocketTilePeekParams(
      ...
      icache = Some(ICacheParams(
        ...
        nTLBWays = 32,
        blockBytes = site(CacheBlockBytes))))
    List.tabulate(n)(i => RocketTilePeekAttachParams(
...
class RocketTilePEEKAttachConfig(f: RocketTilePeekAttachParams => RocketTilePeekAttachParams) extends TileAttachConfig[RocketTilePeekAttachParams](f)
...
class RocketPEEKCrossingConfig(f: RocketCrossingParams => RocketCrossingParams) extends RocketTilePEEKAttachConfig(tp => tp.copy(
  crossingParams = f(tp.crossingParams)
))
...
class WithPEEKCDC(crossingType: ClockCrossingType = SynchronousCrossing()) extends RocketPEEKCrossingConfig(_.copy(crossingType = crossingType))
...
class WithPEEKAsynchronousCDCs(depth: Int, sync: Int)                  extends WithPEEKCDC(AsynchronousCrossing(depth, sync))
class WithPEEKRationalCDCs                    extends WithPEEKCDC(RationalCrossing())
