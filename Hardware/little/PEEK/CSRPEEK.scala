...
class CSRFileIOPEEK(hasBeu: Boolean)(implicit p: Parameters) extends CoreBundle
...
class CSRFilePEEK(
  ...
  hasBeu: Boolean = false)(implicit p: Parameters)
    ...
    with HasCoreParameters {
  val io = IO(new CSRFileIOPEEK(hasBeu) {
    val customCSRs = Vec(CSRFilePEEK.this.customCSRs.size, new CustomCSRIO)
    val roccCSRs = Vec(CSRFilePEEK.this.roccCSRs.size, new CustomCSRIO)
  })
