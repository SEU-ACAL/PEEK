...
class GHE(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCCPEEK(opcodes) {
  override lazy val module = new GHEImp (this)
}

class GHEImp(outer: GHE)(implicit p: Parameters) extends LazyRoCCPEEKModuleImp(outer)
