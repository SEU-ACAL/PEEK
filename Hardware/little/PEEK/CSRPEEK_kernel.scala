...
class CSRFileIOPEEK_kernel(hasBeu: Boolean)(implicit p: Parameters) extends CoreBundle
...
class CSRFilePEEK_kernel(
  perfEventSets: freechips.rocketchip.peek.EventSets = new freechips.rocketchip.peek.EventSets(Seq()),
  customCSRs: Seq[CustomCSR] = Nil,
  roccCSRs: Seq[CustomCSR] = Nil,
  hasBeu: Boolean = false)(implicit p: Parameters)
    extends CoreModule()(p)
    with HasCoreParameters {
  val io = IO(new CSRFileIOPEEK_kernel(hasBeu) {
    val customCSRs = Vec(CSRFilePEEK_kernel.this.customCSRs.size, new CustomCSRIO)
    val roccCSRs = Vec(CSRFilePEEK_kernel.this.roccCSRs.size, new CustomCSRIO)
  })
