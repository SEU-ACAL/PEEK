...
trait HasRocketCoreIOPEEK extends HasRocketCoreParameters {
  implicit val p: Parameters
  def nTotalRoCCCSRs: Int
  val io = IO(new CoreBundle()(p) {
    val hartid = Input(UInt(hartIdLen.W))
    val reset_vector = Input(UInt(resetVectorLen.W))
    val interrupts = Input(new CoreInterrupts(tileParams.asInstanceOf[RocketTilePeekParams].beuAddr.isDefined))
    val imem  = new FrontendIO
    val dmem = new HellaCacheIO
    val ptw = Flipped(new DatapathPTWIO())
    val fpu = Flipped(new FPUCoreIOPEEK())
...
class RocketPEEK(tile: RocketTilePeek)(implicit p: Parameters) extends CoreModule()(p)
...
//
  ...
  val csr = Module(new CSRFilePEEK(perfEvents, coreParams.customCSRs.decls, tile.roccCSRs.flatten, tile.rocketParams.beuAddr.isDefined))
