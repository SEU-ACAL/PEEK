...
trait HasNonDiplomaticTileParameters {
  ...
  def usingRoCC: Boolean = !p(BuildRoCC).isEmpty || !p(BuildRoCCNpu).isEmpty|| !p(BuildRoCCPEEK).isEmpty || !p(BuildRoCCBB).isEmpty // may can be overrided by each one, not in global config
  ...
  def dcacheArbPorts = 1 + usingVM.toInt + usingDataScratchpad.toInt + p(BuildRoCC).size + p(BuildRoCCNpu).size +p(BuildRoCCPEEK).size+ p(BuildRoCCBB).size + (tileParams.core.useVector && tileParams.core.vectorUseDCache).toInt // may can be overrided by each one, not in global config
